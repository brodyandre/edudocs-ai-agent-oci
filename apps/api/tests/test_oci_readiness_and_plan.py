...
def test_plan_audit_accepts_valid_create_plan() -> None:
    ...
    assert plan_check.collect_findings(valid_plan()) == []
    ...
    plan["resource_changes"].append(
        create_change(
            ...
            "oci_core_network_security_group_security_rule",
            {
                ...
                "source": "0.0.0.0/0",
                "tcp_options": [
                    {"destination_port_range": [{"min": 3000, "max": 3000}]}
                ],
            },
        )
    )
...
def test_readiness_masks_and_validates_admin_cidr() -> None:
    ...
    assert {
        finding.kind for finding in readiness.validate_admin_cidr("10.0.0.1/32")
